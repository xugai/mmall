...
//    private Logger log = LoggerFactory.getLogger(OrderServiceImpl.class);
    ...
    public ServerResponse createOrder(Integer userId,Integer shippingId){
        ...
        if(rowCount <= 0){
            return ServerResponse.createByErrorMessage("新创建的订单插入数据库失败！");
        }
        //为每一个订单明细的订单号都赋值成当前订单的订单号
        ...
        //把我们生成的订单数据返回给前端（涉及到POJO与VO对象之间的转化）
    ...
    public ServerResponse cancel(Integer userId,Long orderNo){
        ...
        if(order == null){
            return ServerResponse.createByErrorMessage("当前用户无此订单！");
        }
    ...
    public ServerResponse getOrderCartProduct(Integer userId){
        ...
        if(CollectionUtils.isEmpty(cartList)){
            return ServerResponse.createByErrorMessage("当前用户没有选择结算的商品！");
        }
        ServerResponse serverResponse = this.getCartOrderItem(userId,cartList);
        if(!serverResponse.isSuccess()){
            return serverResponse;
        ...
        orderProductVo.setImageHost(PropertiesUtil.getProperty("ftp.server.http.prefix", "http://img.immall.tk/"));
    ...
    private List<OrderVo> assembleOrderVoList(List<Order> orderList,Integer userId){
        ...
        for (Order order : orderList) {
            ...
            if(userId == null){
                // TODO: 2018/2/23 若当前角色是管理员,则不需要userId
    ...
    private OrderVo assembleOrderVo(Order order,List<OrderItem> orderItemList){
        ...
        if(shipping != null) {
            ...
            ShippingVo shippingVo = this.assembleShipping(shipping);
        ...
        orderVo.setImageHost(PropertiesUtil.getProperty("ftp.server.http.prefix", "http://img.immall.tk/"));
    ...
    private Order assembleOrder(Integer userId,Integer shippingId,BigDecimal payment){
        ...
        // TODO: 2018/2/21 发货时间、付款时间等需要set
        return order;
    }
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        List<OrderItem> orderItemList = Lists.newArrayList();
        if(CollectionUtils.isEmpty(cartList)){
            return ServerResponse.createByErrorMessage("当前用户没有选中的需要购买的商品！");
        }
        //校验购物车的数据,包括产品的状态和数量
    ...
    public ServerResponse<Map> pay(Long orderNo,Integer userId,String path) {
        ...
        String subject = new StringBuilder().append("订单").append(outTradeNo).append("当面付扫码消费").toString();
        ...
        String body = new StringBuilder().append("mmall商城订单").append(outTradeNo).append("消费支付").toString();
    ...
    public ServerResponse callBack_Check(Map<String,String> map){
        Long order_no = Long.valueOf(map.get("out_trade_no"));  //获取回调过来的订单号
        BigDecimal totalAmount = new BigDecimal(map.get("total_amount"));   //获取回调过来的总金额
        String trade_status = map.get("trade_status");     //获取回调过来的交易状态
        String trade_no = map.get("trade_no");      //获取回调过来的支付宝交易号
        ...
        if(!order.getPayment().equals(totalAmount)){
            return ServerResponse.createByErrorMessage("回调的总金额匹配不上！");
        }
...
//        }
        ...
        payInfo.setPlatformNumber(Const.PayPlatformEnum.ALIPAY.getValue());
    ...
    public ServerResponse sendOrderGoods(Long orderNo){
        Order order = orderMapper.selectByOrderNo(orderNo);
        if(order == null){
            return ServerResponse.createByErrorMessage("该订单不存在！");
        }
        //判断该订单是否处于已支付状态,若是则修改订单状态为已发货状态,否则不做处理
        if(order.getStatus() == Const.OrderStatusEnum.PAID.getCode()){
